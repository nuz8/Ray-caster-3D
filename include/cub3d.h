...
typedef struct s_game
{
	...
	mlx_t				*mlx;
	t_img				*background;
	int32_t				background_inst_id;
	t_img				*img_3d;
	int32_t				img_3d_inst_id;
	t_txr				*no_texture;
	t_txr				*so_texture;
	t_txr				*we_texture;
	t_txr				*ea_texture;
...
void					place_lined_block(t_img *img, int x, int y,
							int block_color);
...
void					init_tile_edges(double step, int tile, int *start,
							int *end);
bool					validate_edge(int *start, int *end, int max);
bool					init_direction_ctx(t_player *pl, t_data *data,
							double angle, t_dir_ctx *ctx);
void					draw_direction_line(t_player *pl, t_dir_ctx *ctx,
							uint32_t colour);
int						world_to_minimap(double value, double scale);
void					minimap_plot_ray(t_player *pl, t_rays *ray,
							uint32_t colour);
bool					init_ray_ctx(t_player *pl, t_rays *ray, t_ray_ctx *ctx);
void					plot_ray_pixels(t_player *pl, t_ray_ctx *ctx,
							uint32_t colour);
...
void					column_write_texel(t_column_ctx *ctx, int y,
							uint32_t color);

// src/coordinates

bool					wall_collision_circle(t_map *map, double center_x,
							double center_y);

// src/ray_casting

void					cast_rays(t_map *map, t_rays **rays, t_data *data);
void					initialize_ray_caster(t_rays *ray, t_data *data,
							t_dvec *d_ptr[], t_ivec *check);
void					update_distance(t_dvec *hype, t_ivec *check);
void					extract_hit(t_rays *ray, t_ray_trace *ctx);
void					assign_wall_texture(t_game *game, t_rays *ray,
							t_dvec hop, t_ivec check);
...
void					free_exit_early(t_game *game, char *msg, int ret,
							char *str);
